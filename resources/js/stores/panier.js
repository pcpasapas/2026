        ...
        modifStorePanier(id, payload) {
            ...
            if (payload === 0) {
                ...
                for (let i = 0; i < id.length; i++) {
                    if (id[i] === 0) {
                        this.panier[0].article =
                            useBoitiersStore().getOneBoitier(payload[i])
                    } else if (id[i] === 1) {
                        this.panier[1].article = useAlimsStore().getOneAlim(
                            payload[i]
                        )
                    } else if (id[i] === 2) {
                        this.panier[2].article =
                            useProcesseurStore().getOneProcesseur(payload[i])
                    } else if (id[i] === 3) {
                        this.panier[3].article =
                            useCarteMereStore().getOneCarteMere(payload[i])
                    } else if (id[i] === 4) {
                        this.panier[4].article = useRamStore().getOneRam(
                            payload[i]
                        )
                    } else if (id[i] === 5) {
                        this.panier[5].article =
                            useCartegraphiqueStore().getOnecarteGraphique(
                                payload[i]
                            )
                    } else if (id[i] === 6) {
                        this.panier[6].article = useSsdStore().getOneSsd(
                            payload[i]
                        )
                    } else if (id[i] === 7) {
                        this.panier[7].article = useHddStore().getOneHdd(
                            payload[i]
                        )
                    }
